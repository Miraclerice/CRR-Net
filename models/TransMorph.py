...
Original paper:
Chen J, Frey EC, He Y, Segars WP, Li Y, Du Y (2022) TransMorph: Transformer for unsupervised medical image registration. Med Image Anal 82102615. https://doi.org/10.1016/j.media.2022.102615
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, size, mode='bilinear'):
        ...
        self.register_buffer('grid', grid)  # register the grid as a buffer
...
class TransMorph(nn.Module):
    def __init__(self, config=None, **kwargs):
        ...
        config = get_3DTransMorph_config() if config is None else config
    ...
    def forward(self, src, tgt):
        ...
        out = self.spatial_trans(src, flow)
        return out, flow

if __name__ == '__main__':
    model = TransMorph()
    input = torch.randn(1, 1, 160, 192, 160)
    out, flow = model(input, input.clone())
    ...
    from thop import profile
    macs, params = profile(model, inputs=(torch.randn(1, 1, 160, 192, 160), torch.randn(1, 1, 160, 192, 160)))
