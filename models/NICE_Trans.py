...
Original paper:
Meng M, Bi L, Fulham M, Feng D, Kim J (2023) Non-iterative coarse-to-fine transformer networks for joint affine and deformable image registration. In: Greenspan H, Madabhushi A, Mousavi P, Salcudean S, Duncan J, Syeda-Mahmood T, Taylor R (eds) International Conference on Medical Image Computing and Computer-Assisted Intervention (MICCAI 2023), Springer Nature Switzerland, Vancouver, BC, Canada, 8-12 October 2023. https://doi.org/10.1007/978-3-031-43999-5_71
...
class NICE_Trans(nn.Module):
    ...
    def __init__(self,
                 ...
                 enc_channels: int = 8,
                 dec_channels: int = 16,
                 use_checkpoint: bool = True, **kwargs):
        super().__init__()
...
class ResizeTransformer_block(nn.Module):
    ...
    def forward(self, x):
        ...
        return x  # b, c, d*factor, h*factor, w*factor
...
class AffineHead_block(nn.Module):
    ...
    def forward(self, x_in):
        ...
        self.id[0, 1, 1] = 1
        self.id[0, 2, 2] = 1
        affine_matrix = affine_para.reshape(1, 3, 4) + self.id  # 1, 3, 4
        affine_grid = nnf.affine_grid(affine_matrix, x_in.shape, align_corners=True)  # 1, d, h, w, 3
        ...
        grid = torch.unsqueeze(grid, 0)
        grid = grid.type(torch.FloatTensor)
        grid = grid.to(x_in.device)  # 1, d, h, w, 3

        affine_grid = affine_grid[..., [2,1,0]]  # zxy->xyz
        affine_grid = affine_grid.permute(0, 4, 1, 2, 3)  # 1, 3, d, h, w
        for i in range(len(shape)):
...
class SwinTrans_stage_block(nn.Module):
    ...
    def forward(self, x_in):
        ...
        b, c, d, h, w = x_in.shape  # x_in: B, C, D, H, W
...
class SwinTrans_Block(nn.Module):
    ...
    def forward_part1(self, x_in, mask_matrix):
        ...
        if any(i > 0 for i in shift_size):
            ...
            shifted_x = x
            attn_mask = None
        # W-MSA/SW-MSA d,h,w=dp,hp,wp
        x_windows = window_partition(shifted_x, window_size)  # d*h*w//(wd*wh*ww), wd*wh*ww, c
        attn_windows = self.attn(x_windows, mask=attn_mask)  # d*h*w//(wd*wh*ww), wd*wh*ww, c

        attn_windows = attn_windows.view(-1, *(window_size + (c,)))  # d*h*w//(wd*wh*ww), wd, wh, ww, c
        shifted_x = window_reverse(attn_windows, window_size, dims)  # b, d, h, w, c
        ...
        if pad_d1 > 0 or pad_r > 0 or pad_b > 0:
            x_out = x_out[:, :d, :h, :w, :].contiguous()  # 取消补零

        return x_out  # b, d, h, w, c

    def forward_part2(self, x_in):
        """FFN"""
        x = self.norm2(x_in)  # b, d, h, w, c
        x_out = self.mlp(x)
        return x_out  # b, d, h, w, c
    ...
    def forward(self, x_in, mask_matrix=None):
        ...
        if self.use_checkpoint and x_in.requires_grad:
            x = x_in + checkpoint.checkpoint(self.forward_part1, x_in, mask_matrix)  # b, d, h, w, c
        else:
            x = x_in + self.forward_part1(x_in, mask_matrix)

        if self.use_checkpoint and x.requires_grad:
            x_out = x + checkpoint.checkpoint(self.forward_part2, x)  # FFN b, d, h, w, c
        else:
...
class MSA_block(nn.Module):
    ...
    def __init__(self,
        ...
        relative_coords[:, :, 0] += self.window_size[0] - 1  # d+wd-1
        relative_coords[:, :, 1] += self.window_size[1] - 1  # h+wh-1
        relative_coords[:, :, 2] += self.window_size[2] - 1  # w+ww-1
        relative_coords[:, :, 0] *= (2 * self.window_size[1] - 1) * (2 * self.window_size[2] - 1)  # d*(2wh-1)*(2ww-1)
        relative_coords[:, :, 1] *= 2 * self.window_size[2] - 1  # (2wh-1)*ww
    ...
    def forward(self, x_in, mask=None):
        ...
        b, n, c = x_in.shape  # b=dp*hp*wp//wd*wh*ww,n=wd*wh*ww,c
        qkv = self.qkv(x_in).reshape(b, n, 3, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)  # 3, b, n_head, n, c//n_head
        q, k, v = qkv[0], qkv[1], qkv[2]  # b, n_head, n, c//n_head
        ...
        ].reshape(n, n, -1)  # wd*wh*ww, wd*wh*ww, n_head
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # n_head, wd*wh*ww, wd*wh*ww
        attn = attn + relative_position_bias.unsqueeze(0)  # b, n_head, n, n
...
def compute_mask(dims, window_size, shift_size, device):
    ...
    mask_windows = window_partition(img_mask, window_size)  # d*h*w//(wd*wh*ww), wd*wh*ww, 1
    mask_windows = mask_windows.squeeze(-1)  # d*h*w//(wd*wh*ww), wd*wh*ww
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)  # 广播机制, d*h*w//(wd*wh*ww), wd*wh*ww, wd*wh*ww
...
def window_partition(x_in, window_size):
    ...
    return windows  # d*h*w//(wd*wh*ww), wd*wh*ww, c
...
if __name__ == '__main__':
    ...
    fix = torch.ones(inshape).cuda()
    mov = torch.ones(inshape).cuda()
    model = NICE_Trans().cuda()
    out = model(fix, mov)
    print(out[1].shape)
    from thop import profile
    macs, params = profile(model, inputs=(fix, mov))
