...
Original paper:
Meng M, Feng D, Bi L, Kim J (2024) Correlation-aware coarse-to-fine MLPs for deformable medical image registration. In: 2024 IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), IEEE, Seattle, WA, USA, 16-22 June 2024. https://doi.org/10.1109/CVPR52733.2024.00921
...
class MLP_decoder(nn.Module):
    ...
    def forward(self, x_fix, x_mov):
        x_fix_1, x_fix_2, x_fix_3, x_fix_4 = x_fix  # c, 2c, 4c, 8c  c=enc_c=16
...
class MultiWinMlpLayer(nn.Module):  # input shape: n, h, w, d, c
    ...
    def forward_run(self, x_in):
        ...
        a = self.reweight(a).reshape(n, c, 3).permute(2, 0, 1).softmax(dim=0).unsqueeze(2).unsqueeze(2).unsqueeze(
            2)  # MLP + Softmax
...
class SpatialGatingUnit(nn.Module):  # input shape: n (gh gw gd) (fh fw fd) c
    ...
    def forward(self, x):
        c = x.size(-1)
        c = c // 2
        u, v = torch.split(x, c, dim=-1)  # x.chunk(2, dim=-1)
...
class Correlation(nn.Module):
    def __init__(self, max_disp=1, kernel_size=1, stride=1, use_checkpoint=False):
        ...
        self.padlayer = nn.ConstantPad3d(max_disp, 0)  # D+pf+pb、H+pt+pb、W+pl+pr
    ...
    def forward_run(self, x_1, x_2):
        ...
        x_out = torch.cat(
            [torch.mean(x_1 * x_2[:, :, dx:dx + w, dy:dy + h, dz:dz + d], 1, keepdim=True)
...
if __name__ == '__main__':
    dummy_fix = torch.rand(1, 1, 160, 192, 160)
    dummy_mov = torch.rand(1, 1, 160, 192, 160)
    model = CorrMLP()

    print(sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6)
    from thop import profile

    flops, params = profile(model, inputs=(dummy_fix, dummy_mov))
    print(flops / 1e9 * 2, params / 1e6)
