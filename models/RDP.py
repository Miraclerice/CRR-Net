...
https://github.com/ZAX130/RDP

Original paper:
Wang H, Ni D, Wang Y (2024) Recursive deformable pyramid network for unsupervised medical image registration. IEEE Trans Med Imaging 436:2229-2240. https://doi.org/10.1109/TMI.2024.3362968
...
class VecInt(nn.Module):
    ...
    def forward(self, vec):
        vec = vec * self.scale  # \phi^{(\frac{1}{2^7})}=Id+\frac{1}{2^7}v
        for _ in range(self.nsteps):
            vec = vec + self.transformer(vec,
                                         vec)  # \phi^{(\frac{1}{2^{n-1}})}=\phi^{(\frac{1}{2^n}+\frac{1}{2^n})}=\phi^{(\frac{1}{2^n})}\circ\phi^{(\frac{1}{2^n})}
        return vec
...
class RDP(nn.Module):
    def __init__(self, inshape=(160, 192, 160), flow_multiplier=1., in_channel=1, channels=16, **kwargs):
