...
Original paper:
Wang H, Ni D, Wang Y (2023) ModeT: Learning deformable image registration via motion decomposition transformer. In: Greenspan H, Madabhushi A, Mousavi P, Salcudean S, Duncan J, Syeda-Mahmood T, Taylor R (eds) International Conference on Medical Image Computing and Computer-Assisted Intervention (MICCAI 2023), Springer Nature Switzerland, Vancouver, BC, Canada, 8-12 October 2023. https://doi.org/10.1007/978-3-031-43999-5_70
...
class CWM(nn.Module):
    def __init__(self, in_channels, channels):
        ...
        self.upsample = nn.Upsample(  # torch.nn.functional.interpolate
            scale_factor=2,
            mode='trilinear',
            align_corners=True
        )
...
class ModeTransformer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=3, qk_scale=None, use_rpb=True):
        ...
        grid = torch.stack(grids, -1).type(
            torch.FloatTensor)
        self.register_buffer('grid', grid)

    def makeV(self, N):
        # v.shape: (1, N, self.num_heads, self.kernel_size**3, 3)
        v = self.grid.reshape(self.kernel_size ** 3, 3).unsqueeze(0).unsqueeze(0).repeat(N, self.num_heads, 1,
                                                                                         1).unsqueeze(0)
        return v  # 1, N, self.num_heads, self.kernel_size**3, 3

    def apply_pb(self, attn, N):
        # attn: B, N, self.num_heads, 1, tokens = (3x3x3)
        bias_idx = torch.arange(self.rpb_size ** 3).unsqueeze(-1).repeat(N, 1)  # rpb_size**3*N, 1
        return attn + self.rpb.flatten(1, 3)[:, bias_idx].reshape(self.num_heads, N, 1, self.rpb_size ** 3).transpose(0,
                                                                                                                      1)
    ...
    def forward(self, q, k):
        ...
        q = q.reshape(B, N, self.num_heads, C // self.num_heads, 1).transpose(3,
                                                                              4) * self.scale  # 1, N, num_heads, 1, head_dim
        ...
        k = k.unfold(1, self.kernel_size, 1).unfold(2, self.kernel_size, 1).unfold(3, self.kernel_size, 1).permute(0, 4,
                                                                                                                   5, 6,
                                                                                                                   1, 2,
                                                                                                                   3)  # C, 3, 3, 3, H, W, T
        k = k.reshape(B, self.num_heads, C // self.num_heads, num_tokens,
                      N)  # memory boom B,num_heads,head_dim,num_tokens,N
        k = k.permute(0, 4, 1, 3, 2)  # (B, N, num_heads, num_tokens, head_dim)

        attn = (q @ k.transpose(-2, -1))  # =>B x N x num_heads x 1 x num_tokens
        if self.use_rpb:
            attn = self.apply_pb(attn, N)
        attn = attn.softmax(dim=-1)  # B, N, num_heads, 1, num_tokens

        v = self.makeV(N)  # B, N, num_heads, num_tokens, 3
        x = (attn @ v)  # B x N x num_heads x 1 x 3
        x = x.reshape(B, H, W, T, self.num_heads * 3).permute(0, 4, 1, 2, 3)  # B, num_heads*3, H, W, T
